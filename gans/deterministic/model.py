...
class LSTM(object):
    def __init__(self, num_emb, batch_size, emb_dim, hidden_dim,
                 ...
                 learning_rate=0.01, reward_gamma=0.95, latent_dim=16):
        ...
        def _g_recurrence(i, x_t, h_tm1, gen_o, gen_x):
            ...
            next_token = tf.cast(tf.argmax(log_prob, 1), tf.int32) # deterministic RNN [batch]
            x_tp1 = tf.nn.embedding_lookup(self.g_embeddings, next_token)  # batch x emb_dim
            x_tp1 = tf.concat([self.latent, x_tp1], 1)
        ...
        start_emb = tf.concat([self.latent, start_emb], 1)
        _, _, _, self.gen_o, self.gen_x = control_flow_ops.while_loop(
            cond=lambda i, _1, _2, _3, _4: i < self.sequence_length,
            body=_g_recurrence,
            loop_vars=(tf.constant(0, dtype=tf.int32),
                       start_emb, self.h0, gen_o, gen_x))
        ...
        _, _, _, self.g_predictions, self.g_logits = control_flow_ops.while_loop(
            ...
            loop_vars=(tf.constant(0, dtype=tf.int32), start_emb,
    ...
    def generate(self, session):
        outputs = session.run([self.gen_x], feed_dict={self.latent: np.random.normal(size=(self.batch_size, self.latent_dim))})
        return outputs[0]

    def pretrain_step(self, session, x):
        outputs = session.run([self.pretrain_updates, self.pretrain_loss, self.g_predictions],
            feed_dict={self.x: x, self.latent: np.random.normal(size=(self.batch_size, self.latent_dim))})
        return outputs

    def generator_step(self, sess, samples, rewards):
        feed = {self.x: samples, self.rewards: rewards,
                self.latent: np.random.normal(size=(self.batch_size, self.latent_dim))}
    ...
    def create_recurrent_unit(self, params):
        ...
        self.Wi = tf.Variable(self.init_matrix([self.latent_dim + self.emb_dim, self.hidden_dim]))
        self.Ui = tf.Variable(self.init_matrix([self.hidden_dim, self.hidden_dim]))
        self.bi = tf.Variable(self.init_matrix([self.hidden_dim]))

        self.Wf = tf.Variable(self.init_matrix([self.latent_dim + self.emb_dim, self.hidden_dim]))
        self.Uf = tf.Variable(self.init_matrix([self.hidden_dim, self.hidden_dim]))
        self.bf = tf.Variable(self.init_matrix([self.hidden_dim]))

        self.Wog = tf.Variable(self.init_matrix([self.latent_dim + self.emb_dim, self.hidden_dim]))
        self.Uog = tf.Variable(self.init_matrix([self.hidden_dim, self.hidden_dim]))
        self.bog = tf.Variable(self.init_matrix([self.hidden_dim]))

        self.Wc = tf.Variable(self.init_matrix([self.latent_dim + self.emb_dim, self.hidden_dim]))
