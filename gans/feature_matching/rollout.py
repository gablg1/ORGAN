...
class ROLLOUT(object):
    ...
    def get_reward(self, sess, input_x, data_loader, rollout_num, cnn):
        rewards = []
        for i in range(rollout_num):
            for given_num in range(1, self.sequence_length):
                ...
                neg_h_drop = sess.run(cnn.h_drop, feed)

                positive_samples = data_loader.next_batch()
                feed = {cnn.input_x: positive_samples, cnn.dropout_keep_prob: 1.0}
                pos_h_drop = sess.run(cnn.h_drop, feed) # batch_size x hidden_dim

                loss = np.mean((neg_h_drop - pos_h_drop) ** 2, 1) # batch_size
